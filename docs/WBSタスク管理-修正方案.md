...
| 项目 | 内容 |
|------|------|
| 版本 | **2.0**（Phase 1 / Phase 2 已关门） |
| 日期 | 2026-07-23 |
| 对象 | 项目内的任务（Work Package）管理 |
| 前提 | 不破坏现行 DynamoDB / API / 画面，分阶段引入 |
| 状态 | **Phase 2 已交付并关门 · Phase 3 依赖方案已确认，可开始实施** |
| 相对 0.2 | 字段/汇总/最小 UI 已落地；见 §15 实现纪要 |
| 确认日 | 2026-07-22（§12.1）· Phase 1 关门 2026-07-23 · Phase 2 关门 2026-07-29 |
...
### Phase 2：WBS 构成 + 甘特 — **Done（2026-07-29 关门）**

- 导航增加 **構成 / WBS**（独立第 4 视图）  
- 缩进 / 排序 / 重编号 + move/reorder API  
- **时间线升级为甘特**（自研：树序、父只读汇总条、叶子条整体平移）
- 为避免误触大幅修改预计工时，甘特条不提供左右缘 resize；工时和计划结束日通过详情编辑
...
- `nodeType=milestone`  
- 可选依赖冲突警告  
- 详细方案：→ [`docs/WBS-Phase3-任务依赖开发计划.md`](./WBS-Phase3-任务依赖开发计划.md)
...
- **Phase 1 与 Phase 2 均已关门；下一阶段为 Phase 3 任务依赖开发。**
...
*本文档 2.0：Phase 1 与 Phase 2 均已关门（2026-07-29）。下一阶段见 Phase 3 依赖开发计划。*
