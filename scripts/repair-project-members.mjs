/**
 * 既存プロジェクトの memberEmails を Cognito sub に紐づけ直す
 * Usage: node scripts/repair-project-members.mjs
 */
import {
  ...
  for (const project of items) {
    ...
    for (const email of realEmails) {
      try {
        ...
        if (!found?.userId) {
          console.log(`  [skip] ${project.name}: ${email} not in Cognito`)
          continue
        }
    ...
    console.log(`FIXED ${project.name} memberIds=${[...memberIds].join(',')}`)
  }
}
